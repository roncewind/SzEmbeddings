...
**Important:** Use the same model format for loading AND searching. See [ONNX_MODELS.md](ONNX_MODELS.md) for benchmark results and [../CLAUDE.md](../CLAUDE.md) for usage examples.
...
- 0) Name-only search (baseline - traditional matching)
- 1) Embedding-only search (semantic similarity without name)
- 2) PostgreSQL cosine similarity (direct embedding search)
- 3) Candidate keys (what embeddings retrieved)
- 4) Final ranked entities (name + embedding search)
- 5) Analysis (comparison and diagnostics)
...
- "J Sanz" -> finds "Jose Luis Sanz Ruiz"

*Cross-script queries (semantic matching across scripts):*
- "Alexander Makarov" -> finds "Aleksandr Sergeevich Makarov" (Cyrillic)
- "Ucha Surmanidze" -> finds Arabic variant
- "China Power" -> finds Chinese variant
...
- **[LOADING.md](LOADING.md)** - Detailed loading instructions
- **[POSTGRESQL_TUNING.md](POSTGRESQL_TUNING.md)** - PostgreSQL performance tuning
- **[ONNX_MODELS.md](ONNX_MODELS.md)** - ONNX model evaluation results
